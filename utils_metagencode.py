...
def load_testdata(tf, genome, test_chroms, input_dir):
    ...
    if positives_only:
        data_test = [(window.chrom, window.start, window.stop, 0, bigwig_files,np.append(meta, np.array([cpg.count, cds.count, intron.count, promoter.count, utr5.count, utr3.count], dtype=bool)))
