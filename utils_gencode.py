...
def extract_data_from_bed(args, shift, label):
    ...
    cpg_bed = BedTool('resources/cpgisland.bg.gz')
    ...
    for cpg, cds, intron, promoter, utr5, utr3 in itertools.izip(peaks_cpg_bedgraph,peaks_cds_bedgraph,peaks_intron_bedgraph,peaks_promoter_bedgraph,peaks_utr5_bedgraph,peaks_utr3_bedgraph):
        ...
        data.append((chrom, start, stop, shift_size, bigwig_files, gencode, label))
...
def load_testdata(tf, genome, test_chroms, input_dir):
    ...
    peaks_cpg_bedgraph = genome_bed_test_windows.intersect(cpg_bed, wa=True, c=True)
    peaks_cds_bedgraph = genome_bed_test_windows.intersect(cds_bed, wa=True, c=True)
    peaks_intron_bedgraph = genome_bed_test_windows.intersect(intron_bed, wa=True, c=True)
    peaks_promoter_bedgraph = genome_bed_test_windows.intersect(promoter_bed, wa=True, c=True)
    peaks_utr5_bedgraph = genome_bed_test_windows.intersect(utr5_bed, wa=True, c=True)
    peaks_utr3_bedgraph = genome_bed_test_windows.intersect(utr3_bed, wa=True, c=True)

    if positives_only:
        data_test = [(window.chrom, window.start, window.stop, 0, bigwig_files,np.array([cpg.count, cds.count, intron.count, promoter.count, utr5.count, utr3.count], dtype=bool))
                     for y_sample, window, cpg, cds, intron, promoter, utr5, utr3 in itertools.izip(y, genome_bed_test_windows, peaks_cpg_bedgraph,peaks_cds_bedgraph,peaks_intron_bedgraph,peaks_promoter_bedgraph,peaks_utr5_bedgraph,peaks_utr3_bedgraph)
                     if y_sample]
    else:
        data_test = [(window.chrom, window.start, window.stop, 0, bigwig_files,np.array([cpg.count, cds.count, intron.count, promoter.count, utr5.count, utr3.count], dtype=bool))
                     for window, cpg, cds, intron, promoter, utr5, utr3 in itertools.izip(genome_bed_test_windows, peaks_cpg_bedgraph,peaks_cds_bedgraph,peaks_intron_bedgraph,peaks_promoter_bedgraph,peaks_utr5_bedgraph,peaks_utr3_bedgraph)]
    from iter_meta import DataIterator
    bigwig_rc_order = utils.get_bigwig_rc_order(bigwig_names)
...
def load_beddata(tf, genome, bed_file, input_dir):
    ...
    data_bed = [(window.chrom, window.start, window.stop, 0, bigwig_files, np.array([cpg.count, cds.count, intron.count, promoter.count, utr5.count, utr3.count], dtype=bool))
                for window, cpg, cds, intron, promoter, utr5, utr3 in itertools.izip(bed_filtered, peaks_cpg_bedgraph,peaks_cds_bedgraph,peaks_intron_bedgraph,peaks_promoter_bedgraph,peaks_utr5_bedgraph,peaks_utr3_bedgraph)]
    from iter_meta import DataIterator
    bigwig_rc_order = utils.get_bigwig_rc_order(bigwig_names)
