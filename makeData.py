...
def save_data(windows, x_seq, x_bws, y, beds, bigwigs, output_dir, clobber):
    ...
    print 'Saving bed'
    windows.saveas(output_dir + '/windows.bed.gz')
    f = open(output_dir + '/beds.txt', 'w')
    for bed in beds:
        f.write(bed + '\n')
    f.close()

    f = open(output_dir + '/bigwigs.txt', 'w')
...
def get_data(dnase, genome, fasta, beds, excludes, bigwigs, recurrent):
    ...
    # Get a bed file that overlaps at least one DNase interval
    positive_bed = genome_windows.intersect(dnase_bed, u=True).sort()
    ...
    for n, interval in enumerate(positive_slop_bed):
        if chrom != interval.chrom:
            ...
            for f, bw in enumerate(bigwigs):
                chrom_bws[:, f] = np.array(bw[chrom].value)
        ...
        x_seq[n, begin:end, 0:4] = chrom_s[start:stop, :]
        x_bws[n, begin:end, :] = chrom_bws[start:stop]
...
def make_argument_parser():
    ...
    parser.add_argument('--bigwig', '-w', type=str, required=False, nargs='*',
                        help='1D bigWig files converted to HDF5 files (optional).')
...
def main():
    ...
    windows, x_seq, x_bws, y = get_data(dnase, genome, fasta, beds, excludes, bigwigs, recurrent)
    ...
    save_data(windows, x_seq, x_bws, y, beds, bigwigs, output_dir, clobber)
