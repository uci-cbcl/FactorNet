...
def train(datagen_train, datagen_valid, datagen_test, model, epochs, superepochs, output_dir):
    ...
    train_samples_per_epoch = len(datagen_train)/epochs/utils_gencode.batch_size*utils_gencode.batch_size
...
def main():
    ...
    genome = utils_gencode.load_genome()
    print 'loading ChIP labels'
    chip_bed_list, nonnegative_regions_bed_list = \
        utils_gencode.load_chip(input_dirs, tf)
    print 'loading bigWig data'
    bigwig_names, bigwig_files_list = utils_gencode.load_bigwigs(input_dirs)
    print 'loading meta features'
    meta_names, meta_list = utils_gencode.load_meta(input_dirs)
    print 'making features'
    datagen_train, datagen_valid, datagen_test = \
        utils_gencode.make_features(chip_bed_list,
                                    nonnegative_regions_bed_list,
                                    bigwig_files_list, bigwig_names,
                                    meta_list, genome, epochs, negatives,
                                    valid_chroms, test_chroms)
    print 'building model'
    model = utils_gencode.make_model(len(bigwig_names), 6, num_motifs, num_recurrent, num_dense)
