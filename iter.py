...
class DataIterator(Iterator):
    def __init__(self, data_list, genome, batch_size, seqlen, bigwig_rc_order=None, shift=None, shuffle=True, seed=1337):
        self.data_list = data_list
        if data_list is None or len(data_list) == 0:
            self.num_bigwigs = 0
        else:
            self.num_bigwigs = len(data_list[0][3])
        ...
        if data_list is None or len(data_list) == 0:
            ...
            self.labeled = len(data_list[0]) == 5
        if self.labeled:
            self.num_tfs = len(data_list[0][4])
