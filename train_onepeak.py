...
def train(datagen_train, datagen_valid, datagen_test, model, epochs, superepochs, output_dir):
    ...
    train_samples_per_epoch = len(datagen_train)/epochs/utils_onepeak.batch_size*utils_onepeak.batch_size
...
def main():
    ...
    genome = utils_onepeak.load_genome()
    print 'loading ChIP labels'
    chip_bed_list, nonnegative_regions_bed_list = \
        utils_onepeak.load_chip(input_dirs, tf)
    print 'loading bigWig data'
    bigwig_names, bigwig_files_list = utils_onepeak.load_bigwigs(input_dirs)
    print 'making features'
    datagen_train, datagen_valid, datagen_test = \
        utils_onepeak.make_features(chip_bed_list,
                                    nonnegative_regions_bed_list,
                                    bigwig_files_list, bigwig_names,
                                    genome, epochs, negatives,
                                    valid_chroms, test_chroms)
    print 'building model'
    model = utils_onepeak.make_model(len(bigwig_names), num_motifs, num_recurrent, num_dense)
