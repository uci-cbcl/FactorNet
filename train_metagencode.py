...
def train(datagen_train, datagen_valid, datagen_test, model, epochs, superepochs, output_dir):
    ...
    train_samples_per_epoch = len(datagen_train)/epochs/utils_metagencode.batch_size*utils_metagencode.batch_size
...
def main():
    ...
    genome = utils_metagencode.load_genome()
    print 'loading ChIP labels'
    chip_bed_list, nonnegative_regions_bed_list = \
        utils_metagencode.load_chip(input_dirs, tf)
    print 'loading bigWig data'
    bigwig_names, bigwig_files_list = utils_metagencode.load_bigwigs(input_dirs)
    print 'loading meta features'
    meta_names, meta_list = utils_metagencode.load_meta(input_dirs)
    print 'making features'
    datagen_train, datagen_valid, datagen_test = \
        utils_metagencode.make_features(chip_bed_list,
                                    nonnegative_regions_bed_list,
                                    bigwig_files_list, bigwig_names,
                                    meta_list, genome, epochs, negatives,
                                    valid_chroms, test_chroms)
    print 'building model'
    model = utils_metagencode.make_model(len(bigwig_names), 6 + len(meta_names), num_motifs, num_recurrent, num_dense)
